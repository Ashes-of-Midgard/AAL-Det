...
class SSDNeckCBAM(BaseModule):
    ...
    def __init__(self,
        ...
        super(SSDNeckCBAM, self).__init__(init_cfg)
        ...
        # MODIFIED: CBAM
        self.cbam_out = nn.ModuleList()
        for out_channel in out_channels:
            self.cbam_out.append(CBAMBlock(out_channel))
        # END MODIFIED
    ...
    def forward(self, inputs):
        ...
        for (i, layer) in enumerate(self.extra_layers):
            feat = layer(feat)
            feat_out, attn = self.cbam_out[i+len(inputs)](feat)
            outs.append(feat_out)
            attn_outs.append(attn)
        return tuple(outs), tuple(attn_outs)
        # END MODIFIED
